class CoordonneeInfra:
    ...
    def getAllInfraById(conn,ide):
        cursor = conn.cursor()
        postgreSQL_select_Query = "select ci.idinfra,ci.idcoordonneeinfra,ci.nom,st_astext(ci.coordonnee) as coordonnee, ci.nombre,i.intitule from coordonneeInfra ci natural join infra i where ci.idcoordonneeinfra="+str(ide)
        cursor.execute(postgreSQL_select_Query)
        row = cursor.fetchall()
        simba = CoordonneeInfra(row[0][0],row[0][1],row[0][2],row[0][3],row[0][4],row[0][5])
        return simba
