from object.Lalana import *
from object.CoordonneeInfra import *
from object.Cout import *
...
class LalanaSimba:
    ...
    def getLalanaPriporityPopulation(conn,lala,dist):
        rep = lala[0].getPopulationAkaiky(conn,dist)
        toreturn = lala[0]
        for i in range(1 , len(lala)):
            he = lala[i].getPopulationAkaiky(conn,dist)
            if he > rep:
                rep = he
                toreturn = lala[i]
        return toreturn
