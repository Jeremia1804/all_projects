...
class Fonction:
    ...
    @staticmethod
    def reinserer(queue,noeud):
        if len(queue) == 0:
            queue.append(noeud)
        else:
            Fonction.insert(queue,noeud)

    @staticmethod
    def comparaison(objet):
        return objet.estimation

    @staticmethod
    def insert(queue,noeud):
        index = bisect.bisect_left(queue,noeud.estimation, key=Fonction.comparaison)
        queue.insert(index,noeud)
