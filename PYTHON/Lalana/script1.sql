...
select * from pk where valeur>=45 and valeur<=213;
select * from coordonneeInfra;

select st_distancesphere(ho.coordonnee1,hi.coordonnee2) from
(select  idpk,st_astext(coordonnee) as coordonnee1,idlalana from pk where valeur>=45 and valeur<=213) ho
cross join
(select idcoordonneeInfra,idinfra,st_astext(coordonnee) as coordonnee2,nombre from coordonneeInfra) hi
;

select sum(nombre) from
(select distinct(idcoordonneeInfra) as idcoordonneeInfra from
(select *,st_distancesphere(ci.cooinfra,pk.coopk) as distance from
(select idcoordonneeInfra,idinfra,st_astext(coordonnee) as cooinfra from coordonneeInfra
where idinfra=3
) as ci
cross join
(select idpk,st_astext(coordonnee) as coopk from pk where valeur>=0 and valeur<=359 and idlalana=1) as pk) h
where distance<=45000) j
join coordonneeInfra
on coordonneeInfra.idcoordonneeInfra = j.idcoordonneeInfra

select count(distinct(idcoordonneeInfra)) from
(select *,st_distancesphere(ci.cooinfra,pk.coopk) as distance from
(select idcoordonneeInfra,idinfra,st_astext(coordonnee) as cooinfra from coordonneeInfra
where idinfra=1) as ci
cross join
(select idpk,st_astext(coordonnee) as coopk from pk where valeur>=24 and valeur<=104) as pk) h
where distance<=45000


//Par lalana

select count(idcoo),idlalana from
(select distinct(idcoordonneeinfra) as idcoo,idlalana from
(select *,st_distancesphere(ci.cooinfra,pk.coopk) as distance from 
(select idcoordonneeInfra,idinfra,st_astext(coordonnee) as cooinfra from coordonneeInfra
where idinfra=1) as ci
cross join 
(select coopk,valeur,idlalana from
(select idpk,valeur,st_astext(coordonnee) as coopk from pk) as k
join
lalana
on lalana.idlalana=k.idlalana
order by idlalana) as pk) as hafa
where distance<=30000 group by idlalana,idcoordonneeInfra order by idlalana) as h
group by idlalana;


//populationa
select sum(co.nombre) as somme,h.idlalana from
(select distinct(idcoordonneeinfra) as idcoo,idlalana from
(select *,st_distancesphere(ci.cooinfra,pk.coopk) as distance from 
(select idcoordonneeInfra,idinfra,st_astext(coordonnee) as cooinfra from coordonneeInfra
where idinfra=3) as ci
cross join 
(select coopk,valeur,idlalana from
(select idpk,valeur,st_astext(coordonnee) as coopk from pk) as k
natural join
...
order by idlalana) as pk) as hafa
where distance<=30000 group by idlalana,idcoordonneeInfra order by idlalana) as h
join coordonneeInfra as co on co.idcoordonneeInfra=h.idcoo
group by idlalana;
...
select hop.idlalana,hop.count as hopital,pop.somme,eco.count as ecole from
(select count(idcoo),idlalana from
(select distinct(idcoordonneeinfra) as idcoo,idlalana from
(select *,st_distancesphere(ci.cooinfra,pk.coopk) as distance from 
(select idcoordonneeInfra,idinfra,st_astext(coordonnee) as cooinfra from coordonneeInfra
...
where distance<=30000 group by idlalana,idcoordonneeInfra order by idlalana) as h
join coordonneeInfra as co on co.idcoordonneeInfra=h.idcoo
group by idlalana) as pop
on pop.idlalana=hop.idlalana
join
(select count(idcoo),idlalana from
(select distinct(idcoordonneeinfra) as idcoo,idlalana from
(select *,st_distancesphere(ci.cooinfra,pk.coopk) as distance from 
...
order by idlalana) as pk) as hafa
where distance<=30000 group by idlalana,idcoordonneeInfra order by idlalana) as h
group by idlalana) as eco
on eco.idlalana=hop.idlalana;

drop table coordonneeInfra cascade;
drop table infra cascade;
drop table cout cascade;
drop table lalana cascade;
drop table simba cascade;
drop view details_simba cascade;
drop view priorite cascade;
